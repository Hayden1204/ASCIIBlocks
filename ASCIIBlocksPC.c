...
void loadLevel(){
	...
	if(filePointer){
		...
		if(variableA){
			printf("The level LEVEL.AL2 is not a valid level file. Press a key to continue playing.");
			fclose(filePointer);
			getch();
			return;
		}
		...
		printf("Loaded the current level with LEVEL.AL2. Press a key to continue playing.");
	} else {
		printf("The level LEVEL.AL2 cannot be accessed or does not exist. Press a key to continue playing.");
	}
...
void saveLevel(){
	...
	printf("Saved current level to LEVEL.AL2. Press a key to continue playing.");
	getch();
...
void loadTextures(){
	...
	if(filePointer){
		...
		if(variableA){
			printf("The textures TEXTURES.ABT is not a valid textures file. Press a key to continue playing.");
			fclose(filePointer);
			getch();
			return;
		}
		...
		printf("Loaded the current textures with TEXTURES.ABT. Press a key to continue playing.");
	} else {
		printf("The textures TEXTURES.ABT cannot be accessed or does not exist. Press a key to continue playing.");
	}
...
void saveTextures(){
	...
	printf("Saved current textures to TEXTURES.ABT. Press a key to continue playing.");
	getch();
...
void getCommand(){
	...
	if(!strcmp(playerCommand, "help") || !strcmp(playerCommand, "h")){
		...
		printf("Press a key to continue playing.");
		getch();
	...
	if(!strcmp(playerCommand, "set block") || !strcmp(playerCommand, "sb")){
		...
		if(variableA < 0 || variableA > 255){
			printf("Invalid Block ID. Press a key to continue playing.");
			getch();
		} else {
	...
	if(!strcmp(playerCommand, "teleport") || !strcmp(playerCommand, "t")){
		...
		if(variableA < 1 || variableA > WIDTH || variableB < 1 || variableB > HEIGHT) {
			printf("The X or Y position is invalid. Press a key to continue playing.");
			getch();
		} else {
			level[playerX - 1][playerY - 1] = playerSwapBlock;
	...
	if(!strcmp(playerCommand, "teleport relative") || !strcmp(playerCommand, "tr")){
		...
		if(variableA + playerX < 1 || variableA + playerX > WIDTH || variableB + playerY < 1 || variableB + playerY > HEIGHT) {
			printf("The X or Y position adds up to an invalid value. Press a key to continue playing.");
			getch();
		} else {
			level[playerX - 1][playerY - 1] = playerSwapBlock;
			playerX += (char) variableA;
	...
	printf("Invalid command. Press a key to continue playing.");
	getch();
			...
			case ' ':
				...
				switch(variableA){
					...
					case 8:
						...
						break;
					case 9:
						printf("This feature is not currently implemented into the game. Press a key to continue playing.");
						getch();
						break;
					default:
						break;
...
int main(){
	...
	while(true){
		...
		putchar('#');
		printf("  Welcome to ASCIIBlocks PC Edition!\n");
		for(variableA = 0; variableA < HEIGHT; variableA++){
			putchar('|');
			for(variableB = 0; variableB < WIDTH; variableB++){
